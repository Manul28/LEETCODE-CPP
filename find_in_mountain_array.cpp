...
/*
  -array ki length >=3 then it is mountain array
  -arr[0] < arr[1] < ... < arr[i - 1] < arr[i] it means that the next element is  greater than the previous element and they are in increasing order
  -arr[i] > arr[i + 1] > ... > arr[arr.length - 1] it means that the next element will 
   always be smaller than previous element and they are in decreasing order.
